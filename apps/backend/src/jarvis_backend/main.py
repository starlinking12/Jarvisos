...
into `ModelRouter`, the agent framework, the Voice Engine, the Security Center,
and Phase 5 desktop intelligence/automation. Every other module receives
these as constructor arguments rather than reaching for globals, keeping every
subsystem unit-testable in isolation (see `apps/backend/tests/`).
"""
...
from jarvis_backend.agents.desktop import InputController, UIInspector, WindowManager
...
class OrchestratorBundle:
    ...
    desktop_window_manager: WindowManager | None = None
    desktop_ui_inspector: UIInspector | None = None
    desktop_input_controller: InputController | None = None
...
def build_orchestrator(settings: Settings, database: Database | None = None) -> OrchestratorBundle:
    ...
    desktop_input_controller: InputController | None = None
    try:
        desktop_window_manager = WindowManager()
        desktop_ui_inspector = UIInspector()
        desktop_input_controller = InputController()
        register_desktop_tools(
            tool_registry,
            window_manager=desktop_window_manager,
            ui_inspector=desktop_ui_inspector,
            input_controller=desktop_input_controller,
        )
        logger.info("phase5_desktop_tools_registered")
    except Exception as error:  # noqa: BLE001 - optional desktop deps/platform
        logger.info("phase5_desktop_tools_unavailable", detail=str(error))
        desktop_window_manager = None
        desktop_ui_inspector = None
        desktop_input_controller = None
    ...
    for identity, spec in agent_specs.items():
        if identity == EventSource.AGENT_DESKTOP and desktop_window_manager is not None:
            ...
            agents[identity] = DomainAgent(
                spec, model_router=model_router, tool_executor=tool_executor
            )
    ...
    return OrchestratorBundle(
        ...
        desktop_window_manager=desktop_window_manager,
        desktop_ui_inspector=desktop_ui_inspector,
        desktop_input_controller=desktop_input_controller,
    )
