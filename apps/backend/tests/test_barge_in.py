...
class _FakeRawVad:
    """Fakes the `webrtcvad.Vad.is_speech(bytes, rate)` interface without
    the real native dependency — `RawSpeechDetector` only calls this one
    method, so a fake satisfying that exact shape is sufficient."""
