...
def test_wraparound_write_and_read() -> None:
    ...
    buffer.read(8)  # drain it
    # This write wraps around the internal circular buffer.
...
def test_overflow_overwrites_oldest_data_without_raising() -> None:
    ...
    buffer.write(np.array([6, 7], dtype=np.float32))  # overflows by 2

    result = buffer.read(5)
    # Oldest two samples (1, 2) were overwritten; buffer now holds [3,4,5,6,7].
